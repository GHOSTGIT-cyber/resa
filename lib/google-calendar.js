...
function eventBody(r) {
  ...
  const range = localRange(r.date, r.slot, minutes);
  const description = [
    `Client : ${r.name || "—"}`,
    `Téléphone : ${r.phone || "—"}`,
    `E-mail : ${r.email || "—"}`,
  ...
  const body = {
    // status:confirmed -> un PUT (update) "réveille" un event précédemment annulé
    // (cas résa annulée puis re-confirmée : évite un event fantôme manquant).
    status: "confirmed",
    summary: `eFoil ${r.name || "Réservation"} — ${r.formule || "session"} (${r.participants || 1}p)`,
    description,
    location: b.footer || b.name || "",
  };
  // Couleur de l'event dans l'agenda. GOOGLE_EVENT_COLOR_ID : 1..11 (palette Google),
  // défaut "10" (Basil = vert). Mettre "" pour garder la couleur du calendrier.
  const color = process.env.GOOGLE_EVENT_COLOR_ID ?? "10";
  if (color) body.colorId = String(color).trim();
  if (range) {
    ...
    body.end = { date: addDays(r.date, 1) };
  }
  return body;
}
