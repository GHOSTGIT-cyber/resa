...
function confirmedEmailHtml(r, b) {
  const row = (k, v) =>
    `<tr><td style="padding:9px 14px;color:#5B6B6E;font-size:14px;">${k}</td><td style="padding:9px 14px;text-align:right;font-size:14px;"><b>${esc(v)}</b></td></tr>`;
  const inner =
    `<tr><td style="padding:24px 28px 8px;color:#0F2830;">` +
...
function proposalEmailHtml(r, b, p) {
  ...
  const inner =
    ...
    row("Référence", r.ref) +
    `</table>` +
    `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">Ce créneau vous convient ? Répondez à cet e-mail ou appelez le <a href="tel:${telHref(b.phone)}" style="color:#F4631F;">${esc(b.phone)}</a>.</p>` +
    `</td></tr>` +
    googleBox(b);
  return emailShell(inner, b);
}
...
async function sendClientMail(r, kind, extra) {
  ...
  const b = brandFor(r.siteId);
  const builders = {
    confirmation: { subject: `Réservation confirmée — ${b.name} (${r.ref})`, html: () => confirmedEmailHtml(r, b) },
...
export function sendConfirmation(r) {
  return sendClientMail(r, "confirmation");
}
