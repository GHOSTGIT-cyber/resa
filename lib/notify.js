...
function clientEmailHtml(r) {
  ...
  const inner =
    ...
    `<p style="margin:0 0 16px;font-size:15px;line-height:1.6;">Nous avons bien reçu votre <b>demande de réservation eFoil</b>. Notre équipe vous recontacte très vite pour confirmer la date.</p>` +
    ...
    `</table>` +
    `<p style="margin:16px 0 0;font-size:13px;color:#5B6B6E;"><i>Aucun paiement n'a été effectué</i> — demande sans engagement.</p>` +
    `</td></tr>` +
    GOOGLE_BOX;
...
async function sendEmails(r) {
  ...
  try {
    // import dynamique : pas de dépendance dure si SMTP non configuré
    ...
    const transport = nodemailer.createTransport({
      ...
      // évite que le POST traîne si le serveur SMTP ne répond pas
      connectionTimeout: 10000,
      greetingTimeout: 10000,
      socketTimeout: 15000,
    });
    // Expéditeur affiché. Par défaut = compte SMTP. EMAIL_FROM permet d'afficher une
    // autre adresse (ex. via Brevo/Mailgun où l'utilisateur SMTP n'est pas l'adresse d'envoi).
    const fromAddr = process.env.EMAIL_FROM || process.env.SMTP_USER;
    const from = `"eFoil Côte d'Azur" <${fromAddr}>`;
    const jobs = [];
