...
// Date/heure de l'encaissement, en heure de Paris ("" si inconnue).
function paidWhen(r) {
  if (!r?.paidAt) return "";
  try {
    return new Date(r.paidAt).toLocaleString("fr-FR", {
...
// E-mail « PAIEMENT REÇU » envoyé au client dès que l'acompte est encaissé.
// C'est l'accusé de réception qui manquait : paiement confirmé ET réservation confirmée.
function paymentReceiptHtml(r, b) {
  const row = (k, v) =>
    `<tr><td style="padding:9px 14px;color:#5B6B6E;font-size:14px;">${k}</td><td style="padding:9px 14px;text-align:right;font-size:14px;"><b>${esc(v)}</b></td></tr>`;
  const amount = process.env.SUMUP_AMOUNT || "50 €";
  const when = paidWhen(r);
  const inner =
    `<tr><td style="padding:24px 28px 8px;color:#0F2830;">` +
    `<h1 style="margin:0 0 10px;font-size:22px;color:#1a7f4b;">Paiement reçu &#128181;</h1>` +
    `<p style="margin:0 0 16px;font-size:15px;line-height:1.6;">Merci ${esc(r.name)} ! Nous avons bien reçu votre acompte de <b>${esc(amount)}</b>${when ? ` (le ${esc(when)})` : ""}.<br>Votre réservation est <b>confirmée</b> &#9989; — il ne reste plus qu'à venir rider !</p>` +
    `<div style="margin:0 0 16px;padding:12px 14px;background:#e6f7ec;border-radius:10px;color:#1a7f4b;font-size:15px;text-align:center;">` +
    `<b>&#128181; Acompte payé : ${esc(amount)}</b> &middot; Réservation confirmée &#9989;</div>` +
    `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FCF8F3;border-radius:10px;">` +
    row("Formule", r.formule) +
    row("Date &amp; créneau", `${r.date} à ${r.slot}`) +
    row("Participants", r.participants) +
    row("Référence", r.ref) +
    `</table>` +
    `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">Le solde se règle sur place le jour de la session. Une question ou un imprévu ? Répondez à cet e-mail ou appelez le <a href="tel:${telHref(b.phone)}" style="color:#F4631F;">${esc(b.phone)}</a>.</p>` +
    `<p style="margin:10px 0 0;font-size:13px;color:#5B6B6E;">Conservez cet e-mail : il vaut justificatif de votre acompte.</p>` +
    `</td></tr>` +
    googleBox(b);
  return emailShell(inner, b);
}
...
async function sendClientMail(r, kind, extra) {
  ...
  const builders = {
    ...
    paiement: {
      subject: `Paiement reçu ✅ — réservation confirmée — ${b.name} (${r.ref})`,
      html: () => paymentReceiptHtml(r, b),
    },
  }[kind];
  try {
...
// Accusé de réception du paiement (acompte encaissé). Appelé par lib/paid.js uniquement.
export function sendPaymentReceipt(r) {
  return sendClientMail(r, "paiement");
}
