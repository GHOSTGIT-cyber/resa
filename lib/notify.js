...
function confirmedEmailHtml(r) {
  ...
  const inner =
    ...
    row("Référence", r.ref) +
    `</table>` +
    // Bouton de paiement SumUp (le client paie sur sumup.com) si un lien est configuré.
    (process.env.SUMUP_PAYMENT_LINK
      ? `<p style="margin:18px 0 0;text-align:center;"><a href="${process.env.SUMUP_PAYMENT_LINK}" style="display:inline-block;background:#16a34a;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:999px;font-weight:bold;font-size:15px;">Payer ma session &#128179;</a></p>` +
        `<p style="margin:8px 0 0;text-align:center;font-size:12px;color:#5B6B6E;">Paiement sécurisé par carte via SumUp</p>`
      : "") +
...
// E-mail de PROPOSITION d'un autre créneau, envoyé au client.
// p = { date, slot, message }
function proposalEmailHtml(r, p) {
  ...
  const inner =
    ...
    row("Référence", r.ref) +
    `</table>` +
    `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">Ce créneau vous convient ? Répondez à cet e-mail ou appelez le <a href="tel:+33635305067" style="color:#F4631F;">06 35 30 50 67</a>.</p>` +
    `</td></tr>` +
    GOOGLE_BOX;
  return emailShell(inner);
}
...
function cancelEmailHtml(r) {
  ...
  const inner =
    ...
    row("Référence", r.ref) +
    `</table>` +
    `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">Répondez à cet e-mail ou appelez le <a href="tel:+33635305067" style="color:#F4631F;">06 35 30 50 67</a>.</p>` +
    `</td></tr>` +
    GOOGLE_BOX;
  return emailShell(inner);
}
...
export async function sendConfirmation(r) {
  ...
  try {
    await transport.sendMail({
      from: senderFrom(),
      to: r.email,
      bcc: process.env.OWNER_EMAIL || undefined, // copie à la boîte contact/gérant
...
// p = { date, slot, message }. Renvoie { ok, error? }.
export async function sendProposal(r, p) {
  ...
  try {
    await transport.sendMail({
      ...
      to: r.email,
      replyTo: process.env.EMAIL_REPLY_TO || process.env.EMAIL_FROM || process.env.SMTP_USER,
      subject: `Proposition de créneau — eFoil Côte d'Azur (${r.ref})`,
...
// Envoi ponctuel : e-mail d'ANNULATION au client. Renvoie { ok, error? }.
export async function sendCancellation(r) {
  ...
  try {
    await transport.sendMail({
      ...
      to: r.email,
      replyTo: process.env.EMAIL_REPLY_TO || process.env.EMAIL_FROM || process.env.SMTP_USER,
      subject: `Réservation annulée — eFoil Côte d'Azur (${r.ref})`,
