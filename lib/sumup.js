...
// Détail d'UNE transaction (pour identifier le payeur : e-mail, référence checkout…).
// Renvoie l'objet brut SumUp, ou null. On tente par id puis par transaction_code.
export async function getTransactionDetail(idOrCode) {
  const token = await getAccessToken();
  if (!token || !idOrCode) return null;
  for (const q of [`id=${encodeURIComponent(idOrCode)}`, `transaction_code=${encodeURIComponent(idOrCode)}`]) {
    try {
      const res = await fetch(`${API}/me/transactions?${q}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        const d = await res.json().catch(() => null);
        if (d && (d.id || d.transaction_code)) return d;
      }
    } catch {
...
// Extrait une chaîne « qui a payé » depuis un détail de transaction (best-effort :
// SumUp expose des champs variables selon le canal). On ratisse large.
export function payerHint(detail) {
  if (!detail) return "";
  const bits = [];
  const push = (v) => { if (v && typeof v === "string" && !bits.includes(v)) bits.push(v); };
  push(detail.checkout_reference); // nos checkouts : contient la réf résa
  push(detail.description);
  push(detail?.card?.holder_name);
  push(detail.email);
  push(detail?.customer?.email);
  push(detail?.customer?.name);
  push(detail?.personal_details?.email);
  push(detail?.personal_details?.first_name && detail?.personal_details?.last_name
    ? `${detail.personal_details.first_name} ${detail.personal_details.last_name}` : "");
  return bits.join(" · ");
}
