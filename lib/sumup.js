...
// Vérifie le statut du checkout d'une réservation et passe "payé" si PAID. { paid }.
export async function checkAndMarkPaid(ref) {
  ...
  try {
    ...
    if (status === "PAID") {
      if (!r.paid) update(ref, { paid: true, paidAt: new Date().toISOString() });
